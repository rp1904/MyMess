...
public class UserDaoImpl implements UserDao {
	...
	public User findByUserName(String emailOrMobNo) {
		...
		Criterion criterion = Restrictions.or(Restrictions.eq("email", emailOrMobNo), Restrictions.eq("mobileNumber", emailOrMobNo));
	...
	public List<User> getMembersByMessAndSearch(String searchString, Mess mess, int pageNumber, int pageSize) {
		...
		criteria.setMaxResults(pageSize);
		criteria.add(Restrictions.eq("mess", mess));
		criteria.createAlias("userProfile", "up").add(Restrictions.ilike("up.fullName", searchString.toLowerCase(), MatchMode.START));
	...
	public List<AppMember> getMemberListForAppByMessIdPk(String messIdPk) {
		// TODO Auto-generated method stub
		Session session = sessionFactory.getCurrentSession();
		String qry = "SELECT u.user_id_pk, up.full_name, mm.created_at FROM users as u " + "INNER JOIN user_profiles up ON up.user_profile_id_pk = u.user_profile_id_fk " + "LEFT JOIN messes as m ON m.mess_id_pk = u.mess_id_fk "
				+ "LEFT JOIN meals as ml ON ml.mess_id = m.mess_id_pk AND ml.status = 'OPEN' " + "LEFT JOIN members_meals as mm ON u.user_id_pk = mm.member_id_fk " + "AND mm.meal_id_fk = ml.meal_id_pk "
