...
public class MessServicesImpl implements MessServices {
  ...
  public Mess saveMess(Mess mess) {
    // TODO Auto-generated method stub
    mess.getMessOwner().getUserProfile()
        .setFullName(mess.getMessOwner().getUserProfile().getFirstName() + " "
            + mess.getMessOwner().getUserProfile().getLastName());

    String pass = RandomStringGenerator.getRandomString(5);
    mess.getMessOwner().setPassword(pass);

    String msgBody = EmailTemplates.createLoginCredentialsMessage(mess.getMessName(),
        mess.getMessOwner().getEmail(), pass);

    emailService.sendEmail(mess.getMessOwner().getEmail(), ProjectConstant.WELCOME_EMAIL_SUB,
        msgBody);
